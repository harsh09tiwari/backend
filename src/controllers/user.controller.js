...
const getWatchHistory = asyncHandler(async(req, res) => {
    //    req.User._id    =>   it will give ID in STRING format and moongoose will convert it in object id behind the scene
    const user = await User.aggregate([
        {
            $match : {  //   in Aggregation pipeline moongoose don't work, therefore we have to create object id
                _id : new mongoose.Types.ObjectId(req.user._id)
            }
        },
        {
            $lookup : {
                from : "videos",    //   since the Video is converted into "videos"  due to saving in db
